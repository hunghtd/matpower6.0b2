function [nx, cx, done, rollback, evnts, cb_data, results] = cpf_ilim_event_cb(...
...
%
%   Callback to handle ILIM events, triggered by event function
%   CPF_ILIM_EVENT to indicate the point at which an upper active
...
for i = 1:length(evnts)
    if strcmp(evnts(i).name, 'ILIM') && evnts(i).zero
...
%             end
            ...
            np = size(mpc.branch, 1); %number of pair
        end
        ...
        %% find the pair(s) and which lim(s)
         ...
         for j = 1:np
              idx_imax = mpc.branch(j, BR_STATUS) > 0 & ...
                    abs(I(j) - Imax(j)) < d.mpopt.cpf.i_lims_tol ;
              if idx_imax
                    done.flag = 1;
                    done.msg = sprintf('Branch %d reached the max current flow', j);
              end
         end
