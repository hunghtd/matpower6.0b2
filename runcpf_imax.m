function [res, suc] = ...
    runcpf_imax(basecasedata, targetcasedata, mpopt, fname, solvedcase)
...
ilim        = mpopt.cpf.enforce_i_lims;%% enforce current limits
...
%% to handle current limits
if ilim
    cpf_events = cpf_register_event(cpf_events, 'ILIM', 'cpf_ilim_event', mpopt.cpf.i_lims_tol, 1);
    cpf_callbacks = cpf_register_callback(cpf_callbacks, 'cpf_ilim_event_cb', 60);
end
...
%         sprintf('%f co tim duoc khong', success)
        if ~success
            done.flag = 1;
